...
function push_frame() {
    $CURRENT_FRAME_COUNT=[int]"$(read_from_address ${GLOBAL_CURRENT_FRAME_COUNT_ADDRESS})"
    ...
    $CUR_FRAME_SIZE=$([int]$GLOBAL_FRAME_SIZE + 3)

    $FRAME_END_ADDRESS=[int]$GLOBAL_CURRENT_FRAME_COUNT_ADDRESS - 2 - $CUR_FRAME_SIZE * [int]$CURRENT_FRAME_COUNT
    $FRAME_START_ADDRESS=[int]$FRAME_END_ADDRESS + 1 - $CUR_FRAME_SIZE
...
function pop_frame() {
    ...
    if ($CURRENT_FRAME_COUNT -eq 0) {
        exit_fatal "trying to pop element from empty frame. Kernel will be terminated!"
    }

    # We have 3 lines of extra information for debugging
    $CURRENT_FRAME_SIZE=[int]$GLOBAL_FRAME_SIZE + 3

    $FRAME_START=[int]$GLOBAL_CURRENT_FRAME_COUNT_ADDRESS - 1 - $CURRENT_FRAME_COUNT * $CURRENT_FRAME_SIZE
