    ...
    while read -r LINE; do
        ...
        *)                              LEXEMES_COUNT=0;;
        ...
        if [ -n "$CUR_ERROR" ] || [ $LEXEMES_COUNT -eq 0 ]; then
            compilation_error "Incorrect instruction" "$CUR_ERROR"
            continue
        fi
        ...
        if [ "${CUR_CMD}" = "var" ] || [ "${CUR_CMD}" = "label" ]; then
            ...
            if [ "$LEX_TYPE" != "nam" ]; then
                compilation_error "Incorrect instruction" "${CUR_CMD} expects a name as a parameter"
                continue
            fi

            if [ "${CUR_CMD}" = "var" ]; then
                if contains_element "$CUR_NAME" "${VARIABLES[@]}"; then
                    compilation_error "Variable redefinition" "Variable $CUR_NAME already exists"
                    continue
                fi
                VARIABLES+=("${CUR_NAME}")
                ...
                if contains_element "$CUR_NAME" "${LABELS[@]}"; then
                    compilation_error "Label redefinition" "Label $CUR_NAME already exists"
                    continue
                fi
                LABELS+=("${CUR_NAME}")
...
while true; do
    ...
    jump|jump_if)
        ...
        if [ "$CUR_CMD" = jump ]; then
            ...
            RES_STR="$INSTR_JUMP_IF $CUR_ADDR_TO # jump $CUR_DEBUG_TO"
        fi
        ;;
    write|copy)
        get_next_lexeme
